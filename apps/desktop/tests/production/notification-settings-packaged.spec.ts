...
test("shows not enabled yet in the packaged app and enables after the system request updates macOS status", async () => {
  test.skip(process.platform !== "darwin", "Packaged macOS notification helper coverage only.");
  ...
  try {
    ...
    await expect(window.getByRole("button", { name: "Ask system", exact: true })).toHaveCount(1);
    await expect(window.getByRole("button", { name: "Open System Settings", exact: true })).toHaveCount(0);

    await window.getByRole("button", { name: "Ask system", exact: true }).click();
    ...
    await expect(window.getByRole("button", { name: "Ask system", exact: true })).toHaveCount(0);
    await expect(window.getByRole("button", { name: "Open System Settings", exact: true })).toHaveCount(0);
  } finally {
    await harness.close();
  }
...
test("keeps showing not enabled yet when the system request does not change packaged macOS access", async () => {
  test.skip(process.platform !== "darwin", "Packaged macOS notification helper coverage only.");
  const userDataDir = await makeUserDataDir();
...
test("shows Windows-managed notification access in the packaged Windows app", async () => {
  test.skip(process.platform !== "win32", "Packaged Windows notification settings only.");

  const userDataDir = await makeUserDataDir();
  const workspacePath = await makeWorkspace("notification-settings-packaged-windows-workspace");
  const settingsLogPath = join(userDataDir, "notification-settings-packaged-windows.log");
  const harness = await launchPackagedDesktop(userDataDir, {
    initialWorkspaces: [workspacePath],
    testMode: "background",
    envOverrides: {
  ...
  try {
    ...
    await expect.poll(() => window.evaluate(() => window.piApp.getNotificationPermissionStatus()))
      .toBe("system-managed");
    await expect(window.locator(".settings-view")).toContainText("Managed by Windows");
    await expect(window.getByRole("button", { name: "Ask system", exact: true })).toHaveCount(0);
    const openWindowsSettings = window.getByRole("button", {
      name: "Open Windows notification settings",
      exact: true,
    });
    await expect(openWindowsSettings).toBeVisible();

    await openWindowsSettings.click();
    await expect.poll(() => readSettingsLog(settingsLogPath), { timeout: 5_000 }).not.toBe("");
  } finally {
    await harness.close();
  }
