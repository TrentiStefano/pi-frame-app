...
test("creates and selects a worktree-backed workspace from the desktop UI", async () => {
  ...
  try {
    ...
    await expect(window.locator(".empty-panel")).toContainText("Create a conversation for this folder");
    await expect(window.locator(".empty-panel")).not.toContainText("/Users/");

    await window.locator(".environment-picker__button").click();
    const topbarEnvironmentMenu = window.locator(".environment-picker__menu");
    await expect(topbarEnvironmentMenu).toBeVisible();
    await expect(topbarEnvironmentMenu.getByRole("menuitemradio").filter({ hasText: worktreeWorkspace.name })).toHaveAttribute("aria-checked", "true");
    await topbarEnvironmentMenu.getByRole("menuitemradio", { name: "Local", exact: true }).click();
    await expect(window.locator(".environment-picker__button")).toContainText("Local");
    ...
    await expect(window.getByRole("menuitemradio", { name: /Worktree/ })).toBeVisible();
  } finally {
    await harness.close();
  }
});
