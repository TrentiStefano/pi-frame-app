...
test("shows not enabled yet and enables via the system permission request", async () => {
...
test("shows turned off and opens System Settings when notifications are denied", async () => {
  const userDataDir = await makeUserDataDir();
  ...
  try {
    ...
    await expect(window.locator(".settings-view")).toContainText(
      "System notifications are turned off for pi-frame",
    );
    await expect(window.getByRole("button", { name: "Ask system", exact: true })).toHaveCount(0);
...
test("shows Windows-managed notification access and opens Windows notification settings", async () => {
  test.skip(process.platform !== "win32", "Windows notification settings only.");

  const userDataDir = await makeUserDataDir();
  const workspacePath = await makeWorkspace("notification-settings-windows-workspace");
  const settingsLogPath = join(userDataDir, "notification-settings-windows.log");
  const harness = await launchDesktop(userDataDir, {
    initialWorkspaces: [workspacePath],
    testMode: "background",
    envOverrides: {
