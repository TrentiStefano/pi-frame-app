import type { ExtensionCompatibilityIssue } from "@pi-frame/session-driver";
...
export function genericUnsupportedCapabilityMessage(capability: string): string {
  return `Terminal-only ${labelForCapability(capability)} is not supported in the desktop app. Use pi in the terminal for that workflow.`;
}

export function commandUnsupportedCapabilityMessage(commandName: string, capability: string): string {
  return `/${commandName} requires terminal-only ${labelForCapability(capability)} and is not supported in the desktop app yet. Use pi in the terminal for this command.`;
}
