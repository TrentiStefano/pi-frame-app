...
test("captures microphone audio and transcribes it with the bundled Mandarin model", async () => {
  test.setTimeout(90_000);
  const desktopDir = resolve(__dirname, "..", "..");
  const modelDir = join(desktopDir, ".cache", "voice-model", "sherpa-onnx-paraformer-zh-2023-09-14");
  const userDataDir = await makeUserDataDir();
  const agentDir = join(userDataDir, "agent");
  const workspacePath = await makeWorkspace("native-voice-input-workspace");
  ...
  const harness = await launchDesktop(userDataDir, {
    ...
    envOverrides: {
      ...
      PI_APP_VOICE_MODEL_DIR: modelDir,
    },
  });

  try {
    const window = await harness.firstWindow();
    await createNamedThread(window, "Native voice input session");
